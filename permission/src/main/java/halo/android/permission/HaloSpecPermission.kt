...
import halo.android.permission.common.*
import halo.android.permission.spec.*
...
object HaloSpecPermission {
    ...
    private fun checkAndRequest(fragment: Fragment, spec: SpecPermission) {
        if (spec.isGrandOrThrow(fragment.requireContext())) {
            ...
            spec.listener.showRationalView(object : SpecialListener.Process {
                override fun onSpecialNext() {
                    SpecPermissionFragmentCaller(fragment)
                            .requestSpecialPermission(spec)
                }

                override fun onSpecialCancel() {
                    //nothing
                }
            })
